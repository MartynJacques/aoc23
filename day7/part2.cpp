...
HandType get_type(std::string& hand) {
  ...
  if (hand.find('J') == std::string::npos) {
    ...
    try {
      if (char_counts.at('J') == 5) {
        // std::cout << "all Js!" << '\n';
        return five_of_a_kind;
      }
    } catch (std::out_of_range& e) {
      // do nothing, no J found
    }

    try {
      if (char_counts.at('J') == 4) {
        // std::cout << "all Js!" << '\n';
        return five_of_a_kind;
      }
    } catch (std::out_of_range& e) {
...
int main() {
  ...
  std::sort(hands_and_bids.begin(), hands_and_bids.end(),
              ...
              if (a_type == b_type) {
                ...
                for (int i = 0; i < a.first.size(); i++) {
                  if (a.first.at(i) == b.first.at(i)) {
                    continue;
                  } else if (card_ranks.at(a.first.at(i)) >
                             card_ranks.at(b.first.at(i))) {
                    return false;
                  } else {
                    return true;
                  }
